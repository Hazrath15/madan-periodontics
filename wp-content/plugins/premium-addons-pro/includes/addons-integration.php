...
class Addons_Integration {
	...
	public function check_instagram_token() {
		...
		$original_token = sanitize_text_field( wp_unslash( $_GET['token'] ) );

		$token = get_option( 'papro_insta_feed_' . substr( $original_token, 0, 6 ), $original_token );

		$transient_name = 'papro_insta_feed_' . $original_token;
		...
		if ( false === $cache ) {
			...
			update_option( 'papro_insta_feed_' . substr( $original_token, 0, 6 ), $refreshed_token );
		}
	...
	public function register_frontend_scripts() {
		...
		wp_register_script(
			'pa-gsap',
			PREMIUM_PRO_ADDONS_URL . 'assets/frontend/' . $dir . '/pa-gsap' . $suffix . '.js',
			array( 'jquery' ),
			PREMIUM_PRO_ADDONS_VERSION,
			true
		);
		...
		wp_localize_script(
			...
			array(
				...
				'anime_js'      => PREMIUM_PRO_ADDONS_URL . 'assets/frontend/' . $dir . '/anime' . $suffix . '.js',
				'blob_url'      => PREMIUM_PRO_ADDONS_URL . 'assets/frontend/' . $dir . '/premium-blob' . $suffix . '.js',
				'path_url'      => PREMIUM_PRO_ADDONS_URL . 'assets/editor/js/generate-path.js',
			)
		);
