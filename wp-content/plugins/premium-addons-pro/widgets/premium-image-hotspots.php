	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$repeater->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					...
					'{{WRAPPER}} {{CURRENT_ITEM}} img.premium-image-hotspots-image-icon, {{WRAPPER}} {{CURRENT_ITEM}} > svg, {{WRAPPER}} {{CURRENT_ITEM}} .premium-lottie-animation svg' => 'width:{{SIZE}}{{UNIT}} !important; height: {{SIZE}}{{UNIT}} !important',
				),
				'condition'  => array(
					'premium_image_hotspots_icon_type_switch!' => 'text',
				),
		...
		$this->add_control(
			...
			array(
				...
				'prevent_empty' => false,
				'title_field'   => 'Hotspot: <# if ( "font_awesome_icon" === premium_image_hotspots_icon_type_switch ) { #>
		...
		$this->add_responsive_control(
			'width',
			array(
				'label'          => esc_html__( 'Width', 'premium-addons-pro' ),
				'type'           => Controls_Manager::SLIDER,
				'default'        => array(
					'unit' => '%',
				),
		...
		$this->add_responsive_control(
			'space',
			array(
				'label'          => esc_html__( 'Max Width', 'premium-addons-pro' ),
				'type'           => Controls_Manager::SLIDER,
				'default'        => array(
					'unit' => '%',
				),
		...
		$this->add_responsive_control(
			'height',
			array(
				'label'          => esc_html__( 'Height', 'premium-addons-pro' ),
				'type'           => Controls_Manager::SLIDER,
				'default'        => array(
					'unit' => 'px',
				),
		...
		$this->add_responsive_control(
			'object-fit',
			array(
				'label'     => esc_html__( 'Object Fit', 'premium-addons-pro' ),
				'type'      => Controls_Manager::SELECT,
				'condition' => array(
					'height[size]!' => '',
				),
				'options'   => array(
					''        => esc_html__( 'Default', 'premium-addons-pro' ),
					'fill'    => esc_html__( 'Fill', 'premium-addons-pro' ),
					'cover'   => esc_html__( 'Cover', 'premium-addons-pro' ),
					'contain' => esc_html__( 'Contain', 'premium-addons-pro' ),
				),
				'default'   => '',
				'selectors' => array(
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .premium-image-hotspots-main-icons > svg' => 'fill: {{VALUE}};',
				),
			)
		);
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .premium-image-hotspots-main-icons .premium-image-hotspots-image-icon,
					 {{WRAPPER}} .premium-lottie-animation svg' => 'width:{{SIZE}}{{UNIT}} !important; height: {{SIZE}}{{UNIT}} !important',
				),
			)
		);
	...
	protected function render() {
		...
		if ( ! empty( $settings['premium_image_hotspots_tooltips_position'] ) ) {
			if ( is_array( $settings['premium_image_hotspots_tooltips_position'] ) ) {
				...
				$tooltip_postion = explode( ',', $settings['premium_image_hotspots_tooltips_position'] );
			}
		} else {
			$tooltip_postion = array( 'right', 'left' );
		}
				...
				<div <?php echo wp_kses_post( $this->get_render_attribute_string( $list_item_key ) ); ?>>
					...
					if ( 'font_awesome_icon' === $icon_type ) :
						...
						if ( $is_new || $migrated ) :
							...
							?>
							<i class="premium-image-hotspots-icon <?php echo esc_attr( $item['premium_image_hotspots_font_awesome_icon'] ); ?>" aria-hidden="true"></i>
						<?php endif; ?>
		...
		<#
			...
			if ( '' !== settings.premium_image_hotspots_tooltips_position ) {
				if ( Array.isArray( settings.premium_image_hotspots_tooltips_position ) ) {
					...
					tooltipPostion = settings.premium_image_hotspots_tooltips_position.split(',');
				}
			} else {
			...
			<div {{{ view.getRenderAttributeString('image_wrap') }}}>
				...
				_.each( settings.premium_image_hotspots_icons, function( hotspot, index ) {
					...
					if ( iconType === 'font_awesome_icon' ) {
						var iconHTML = elementor.helpers.renderIcon( view, hotspot.new_img_hotspot_icon, { 'class': 'premium-image-hotspots-icon' , 'aria-hidden': true }, 'i' , 'object' ),
				...
				<div {{{ view.getRenderAttributeString(listItemKey) }}}>
					...
					<# } #>
						...
						<# if ( 'font_awesome_icon' === iconType ) { #>
							...
							<#  } else { #>
								<i class =" {{ hotspot.premium_image_hotspots_font_awesome_icon }} premium-image-hotspots-icon " aria-hidden="true"></i>
							<# } #>
